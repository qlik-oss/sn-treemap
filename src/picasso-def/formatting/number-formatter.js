...
// Scavanged from sense-client on 2020-03-11 + add escapeRegExp
...
//		scientific = /e[\+\-][0-9]+/,
const radix = /^\(r(0[2-9]|[12]\d|3[0-6])\)/i;
...
const bin = /^\(bin\)/i;
const rom = /^\(rom\)/i;
const functional = /^(\(rom\)|\(bin\)|\(hex\)|\(dec\)|\(oct\)|\(r(0[2-9]|[12]\d|3[0-6])\))/i;
...
function formatRoman(value, pattern) {
  let i;
  let s = '';
  let v = Number(String(value).slice(-3));
  const nThousands = (value - v) / 1000;
  const decimal = [0, 1, 4, 5, 9, 10, 40, 50, 90, 100, 400, 500, 900].reverse();
  const numeral = [
    '0',
    'I',
    'IV',
...
class NumberFormatter {
  /** @lends module:assets/objects/utils/number-formatter~NumberFormatter# */
  /**
   * @constructs
  ...
  prepare(pattern, t, d) {
    ...
    const isFunctional =
      prep.positive.isFunctional &&
      (!prep.negative || (prep.negative && prep.negative.isFunctional));
    ...
    if (prep.isFunctional) {
      ...
      if (prep.abbreviate) {
        ...
        // value and lower abbreviation is for values below 0.1 (move to the right ==> )
          ...
          // lower abbrevaition is smaller than exponent and we can't get away with not abbreviating
            ...
            !(upperAbbreviation > 0 && -exponent <= prep.maxPrecision)
          ) {
            // (move to left <==)
            suggestedAbbrExponent = lowerAbbreviation;
          }
      ...
      if (!decimalPartPattern && numericPattern.slice(-1)[0] === '#') {
        if (absValue >= 10 ** temp || absValue < 1 || absValue < 1e-4) {
          ...
          temp = Math.max(
            0,
            Math.min(20, temp - Math.ceil(Math.log(absValue) / Math.log(10))),
          );
          for (i = 0; i < temp; i++) {
            numericPattern += '#';
          }
        ...
        value = absValue
          ? absValue.toExponential(15).replace(/\.?0+(?=e)/, '')
          : '0';
      } else {
        const wholePart = Number(
          value
            .toFixed(
              Math.min(20, decimalPartPattern ? decimalPartPattern.length : 0),
            )
            .split('.')[0],
        );
        ...
        if (decimalPartPattern) {
          const nDecimals = Math.max(
            0,
            Math.min(14, decimalPartPattern.length),
          ); // the length of e.g. 0000#####
          ...
          let decimalPart = Number(
            (absValue % 1).toPrecision(nDecimals),
          ).toFixed(nDecimals);
...
function preparePattern(o, t, d) {
  ...
  const regex = createRegExp(t, d);
  let numericPattern = pattern.match(regex);
  numericPattern = numericPattern ? numericPattern[0] : '';
  const prefix = numericPattern
    ? pattern.substr(0, pattern.indexOf(numericPattern))
    : pattern;
  const postfix = numericPattern
    ? pattern.substring(pattern.indexOf(numericPattern) + numericPattern.length)
    : '';
  ...
  o.numericRegex = new RegExp(
    `${escape(t, null, true)}|${escape(d, null, true)}`,
    'g',
  );
