...
function parseInterval(days, pattern) {
  const units = parseIntervalDays(days);
  const d = units.d;
  let h = units.h;
  let m = units.m;
  let s = units.s;
...
class DateFormatter {
  /** @lends module:assets/objects/utils/date-formatter~DateFormatter# */
  /**
   * @constructs
   * @param {Object} localeInfo
   * @param {String} pattern
   */
  constructor(localeInfo, pattern) {
   ...
   */
  format(date, pattern) {
    // TODO - default fallback when pattern is not given. (mek)
    if (!pattern) {
...
DateFormatter.parseInterval = parseIntervalDays;
